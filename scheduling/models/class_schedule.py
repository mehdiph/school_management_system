...
class ClassSchedule(models.Model):
    ...
    bell = models.ForeignKey(Bell,on_delete=models.PROTECT, null=True, blank=True, verbose_name='زنگ')
    created_at = jmodels.jDateTimeField(auto_now_add=True, verbose_name='تاریخ ایجاد')

    class Meta:
        verbose_name = 'برنامه کلاسی'
        verbose_name_plural = 'برنامه‌های کلاسی'
        ordering = ['day_of_week', 'bell__start_time']
        constraints = [
            models.UniqueConstraint(
                fields=['class_room', 'day_of_week', 'week_type', 'bell'],
                name='uniq_class_schedule_slot'
            )
        ]

    def __str__(self):
        return f"{self.class_room.school_class.grade.name}-{self.class_room.school_class.section}-{self.class_room.subject}"
    ...
    def clean(self):
        ...
        overlap_qs = ClassSchedule.objects.filter(
            ...
            bell__start_time__lt=self.bell.end_time,
            bell__end_time__gt=self.bell.start_time,
        )
